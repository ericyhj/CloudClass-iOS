...
@objc public enum AgoraEduContextRenderMode : Int {
    ...
    // Fit: Uniformly scale the video until one of its dimension fits the boundary (zoomed to fit). Areas that are not filled due to the disparity in the aspect ratio are filled with black.
    case hidden, fit
}

@objcMembers public class AgoraEduContextRenderConfig: NSObject {
    public var renderMode: AgoraEduContextRenderMode = .hidden
    
    public init(renderMode: AgoraEduContextRenderMode) {
        super.init()
...
@objcMembers public class AgoraEduContextVideoConfig: NSObject {
    // 视频宽
    public var videoDimensionWidth: Int = 320
    // 视频高
    public var videoDimensionHeight: Int = 240
    // 视频帧率
    public var frameRate: Int = 15

    public init(videoDimensionWidth: Int,
